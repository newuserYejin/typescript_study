...
// 객체 형태로 만들것이지만 필드명과 값의 타입이 정확히 정해지지 않았어도 key는 string , value는 number일 것이다로 생성
...
// type FruitColor = Record<'apple' | 'orange' | string, string>;
...
// 1번. 전자상거래 플랫폼에서 지역 코드에 따른 배송비를 계산하는 로직을 작성하세요.
...
// 배송비 계산 함수 작성
function calculateShippingCost(region: string, costs: Record<RegionCode, number>): number {
  return costs[region];
}
