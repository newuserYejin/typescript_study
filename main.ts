...
// 문제 2번 변수 선언과 초기값 지정
let productName: string = '예에'; // 상품 이름
...
function updateUser(user: { name: string; age?: number }): { name: string; age: number } {
  ...
  const result = { ...user, age: user.age ? user.age : 18 };
...
// proudcts 타입정의  필요
const productsList: { name: string; price: number; category?: string }[] = [
...
//매개변수, 리턴 타입 정의 필요
function getProductsByCategory(bookCategory: string): string[] {
  // 여기에 구현
  return productsList
