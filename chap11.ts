...
type test1 = IsString<number>; // type test1에 "no" 가 들어가는 것을 확인 가능
...
type Subset<T> = {
  // keyof ==> T 의 키 모든 값을 가져온다
  [K in keyof T]?: T[K];
};

const Movie1: Subset<Movie> = { title: '기생충', genre: '액션' };
const Movie2: Subset<Movie> = { rate: 4 };
...
// 3번. 다음 조건을 만족하는 조건부 타입과 함수를 작성하세요.
...
// 조건부 타입 정의
// 무슨 배열이든 배열이면 되기 때문에 Array 뒤에 <>에는 any
